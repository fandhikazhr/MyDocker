...
output_folder = "output_detection_camera"
os.makedirs(output_folder, exist_ok=True)

# Fungsi untuk deteksi objek dari kamera secara real-time
def detect_objects_from_camera():
    ...
    print("🔹 Menjalankan deteksi... Tekan 'q' untuk berhenti.")
    ...
    while True:
        ...
        for i in range(len(scores)):
            if scores[i] > 0.3:  # Hanya deteksi dengan skor > 30%
                class_id = classes[i]
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            print("\n🔹 Deteksi dihentikan oleh pengguna.")
            break

    cap.release()
    cv2.destroyAllWindows()
    print("\n✅ Semua frame telah disimpan di folder 'output_detection/'")
